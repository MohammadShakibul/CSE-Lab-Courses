%Clears the command window, workspace variables, and all open figures to start fresh.
clc
clear
close all


opts1 = detectImportOptions('train.csv','PreserveVariableNames',true); %Automatically detects the structure of the CSV files,Retains original column names
opts1 = setvartype(opts1, 'char'); %Forces all columns to be read as character arrays to ensure consistency across MATLAB versions and operating systems, avoiding automatic type inference issues.
opts2 = detectImportOptions('test.csv','PreserveVariableNames',true);
opts2 = setvartype(opts2, 'char');

train_ref = readtable('train.csv',opts1); %Loads the CSV files into tables
test_ref = readtable('test.csv',opts2);


train = table2cell(train_ref); %Convert tables to cell arrays for easier manipulation
...
writematrix(challenge_output,'output.csv')